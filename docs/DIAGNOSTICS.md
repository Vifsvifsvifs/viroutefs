...
ViRouteFS `0.2.0-alpha` includes real basic network diagnostics that explain results in plain Russian: what happened, technical details, elapsed time when available, and what the user should check next.
...
Current limitation: ViRouteFS uses Android's system DNS resolver in `0.2-alpha`. The custom DNS server field is kept for the product flow, but direct querying of the selected DNS server will be connected later. The app shows this limitation directly on the DNS screen.
...
The Route Simulator remains mock/simulation-only. It is a UX model for explaining future routing decisions, not a real routing engine connected to Android VPN traffic.
