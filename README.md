...
ViRouteFS 0.8.12-alpha prepares clean local abstractions for a future VLESS TCP bridge. It introduces TCP session identity, metadata, lifecycle states, a session manager for create/lookup/counter/close/idle-cleanup operations, and bridge interfaces only. Packet route previews can now display "Would create TCP session" for TCP packet metadata while remaining observation-only and not creating sessions. The VPN Runtime screen shows active TCP session count, per-state statistics, and the empty state "No TCP sessions" because no real runtime sessions are created yet.

There is still no runtime forwarding, no Android traffic proxying, no packets written back to TUN, no REALITY, no UDP bridge, no DNS proxying, no automatic uploads, and no telemetry/cloud upload.
...
ViRouteFS 0.8.8-alpha adds an explicit manual plain-TCP VLESS protocol probe on the VLESS profile screen. Users choose a target host and port, tap **Run VLESS probe**, and the app validates the profile, connects to the configured VLESS server, builds one minimal VLESS TCP request frame with the existing builder, sends that frame, waits briefly for close/error/timeout, and closes the socket. The probe does not send HTTP payloads, does not proxy DNS, does not forward Android traffic, and does not write packets back to TUN. TLS/REALITY transport is not implemented yet. Probe history stays local in app no-backup storage, is capped at 20 per profile, and excludes UUID values and raw frame bytes. See [`docs/VLESS_PROTOCOL.md`](docs/VLESS_PROTOCOL.md).
