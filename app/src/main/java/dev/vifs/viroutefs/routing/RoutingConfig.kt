...
fun validateRoutingConfig(config: RoutingConfig): List<String> = buildList {
    ...
    config.rules.forEach { rule ->
        ...
        if (rule.type == RouteRuleType.CIDR) {
            rule.matchers.filterNot { it.isValidCidr() }.forEach { add("Правило ${rule.name}: некорректный CIDR $it.") }
        }
    }
...
fun isValidCidr(text: String): Boolean = text.isValidCidr()

fun String.isValidCidr(): Boolean {
    val parts = split('/')
    if (parts.size != 2) return false
    val prefix = parts[1].toIntOrNull()?.takeIf { it in 0..32 } ?: return false
    val octets = parts[0].split('.')
    return prefix in 0..32 && octets.size == 4 && octets.all { it.toIntOrNull()?.let { value -> value in 0..255 } == true }
}
