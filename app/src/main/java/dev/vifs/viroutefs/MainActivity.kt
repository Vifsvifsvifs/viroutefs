...
private fun DashboardScreen(
    ...
    ScreenList(padding) {
        ...
        item {
            CompactCard(
                title = "0.4.1-alpha",
                simple = "Профили подключений настраиваются отдельно от правил маршрутизации.",
                details = "versionCode 5 • локальная конфигурация • без телеметрии",
            )
        }
        item {
            CompactCard(
                title = "Приватность",
                simple = "Все проверки запускаются пользователем. Скрытого перехвата, облачной отправки логов и аналитики нет.",
                details = "Будущие PCAP/логи остаются локально до явного экспорта.",
            )
        }
        item {
            CompactCard(
                title = "Статус конфигурации",
                simple = if (loaded) "Локальная конфигурация загружена." else "Загрузка…",
                details = message ?: "Готово.",
            )
        }
    }
}
    ...
    ScreenList(padding) {
        ...
        item {
            CardBlock {
                ...
                Text(
                    text = "В этой версии переключатель управляет демонстрационным состоянием. Реальное Android VpnService-подключение будет добавлено позже.",
        ...
        if (showAdd) {
            item {
                CardBlock {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        listOf("QR-код", "Буфер", "Файл", "Вручную").forEach { option ->
                    ...
                    Button(
                        onClick = {
                            ...
                            val profile = TunnelProfile(
                                ...
                                type = type,
                                description = description.ifBlank { "Профиль добавлен вручную. Движок пока не реализован." },
    ...
    CardBlock {
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = "${profile.type.label} • ${if (profile.enabled) "включён" else "выключен"} • $dns",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
            Switch(
                ...
                onCheckedChange = {
                    onConfig(
                        config.copy(profiles = config.profiles.map { if (it.id == profile.id) it.copy(enabled = !it.enabled) else it }),
                        null,
                    )
                },
            )
        ...
        if (config.defaultProfileId == profile.id) {
            AssistChip(onClick = {}, label = { Text("Основной") })
        } else {
            OutlinedButton(onClick = { onConfig(config.copy(defaultProfileId = profile.id), "Основной профиль изменён.") }) {
                Text("Сделать основным")
            }
        }
        if (profile.mockOnly) {
            Text("Демонстрационный профиль: реальный движок не запускается.", style = MaterialTheme.typography.bodySmall)
        }
        profile.warningText?.let { warning ->
            ...
            OutlinedButton(
                ...
                onClick = {
                    ...
                    val nextDefaultProfileId = if (config.defaultProfileId == profile.id) {
                        remainingProfiles.firstOrNull { it.type == TunnelType.Direct }?.id ?: remainingProfiles.firstOrNull()?.id
                    } else {
        ...
        if (edit) {
            ...
            Button(
                onClick = {
                    onConfig(
                        config.copy(profiles = config.profiles.map { if (it.id == profile.id) it.copy(name = name, description = description) else it }),
                        "Профиль обновлён.",
                    )
                    edit = false
                },
...
private fun RoutesScreen(
    ...
    val routeEngine = remember(config) { RouteEngine(config) }
    var input by rememberSaveable { mutableStateOf("telegram") }
    var decision by remember(config) { mutableStateOf(routeEngine.simulate(input)) }
    ...
    ScreenList(padding) {
        ...
        item {
            CardBlock {
                ...
                Button(onClick = { decision = routeEngine.simulate(input) }) { Text("Показать маршрут") }
        ...
        items(config.profiles, key = { it.id }) { profile ->
            RouteProfileRulesCard(profile, config, onConfig)
        }
    }
}

@Composable
private fun RouteProfileRulesCard(
    profile: TunnelProfile,
    config: RoutingConfig,
    onConfig: (RoutingConfig, String?) -> Unit,
) {
    ...
    val appCount = apps.sumOf { rule -> rule.matchers.size + rule.appMatchers.size }
    ...
    CardBlock {
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = "Apps: $appCount • Sites: ${domains.sumOf { it.matchers.size }} • IP/CIDR: ${cidrs.sumOf { it.matchers.size }}",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
        ...
        if (expanded) {
            MatcherSection("Приложения", apps, appText, { appText = it }, "Добавить приложение") {
                addRule(config, profile.id, RouteRuleType.APP, appText, RoutingConfigDefaults.SYSTEM_DNS_ID, onConfig)
                appText = ""
            }
            MatcherSection("Сайты / домены", domains, domainText, { domainText = it }, "Добавить домен") {
                addRule(config, profile.id, RouteRuleType.DOMAIN, domainText, profile.dnsPolicyId, onConfig)
                domainText = ""
            }
            MatcherSection("IP / CIDR", cidrs, cidrText, { cidrText = it }, "Добавить IP/CIDR") {
                addRule(config, profile.id, RouteRuleType.CIDR, cidrText, profile.dnsPolicyId, onConfig)
                cidrText = ""
            }
            rules.filter { it.type != RouteRuleType.DEFAULT }.forEach { rule ->
                Row(
                    ...
                    Text(
                        text = rule.matchers.joinToString().ifBlank { rule.appMatchers.joinToString { it.displayName ?: it.value } },
                        modifier = Modifier.weight(1f),
                        style = MaterialTheme.typography.bodySmall,
                    )
                    OutlinedButton(
                        onClick = { onConfig(config.copy(rules = config.rules.filterNot { it.id == rule.id }), "Маршрут удалён.") },
                    ) { Text("×") }
                }
            }
        }
...
private fun addRule(
    ...
    val rule = RouteRule(
        ...
        name = "$matcher → $profileId",
        type = type,
        targetProfileId = profileId,
        dnsPolicyId = dnsPolicyId,
        priority = 100 + config.rules.size,
        matchers = listOf(matcher),
        appMatchers = if (type == RouteRuleType.APP) listOf(AppMatcher(AppMatcherPlatform.Any, matcher, matcher)) else emptyList(),
        reason = "Добавлено пользователем в компактном редакторе.",
        technicalDetails = "Скрытые технические поля заполнены автоматически.",
        recommendedAction = "Проверьте результат симулятором.",
    )
...
private fun MatcherSection(
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
        OutlinedTextField(value = value, onValueChange = onValue, label = { Text(title) }, modifier = Modifier.weight(1f))
...
private fun DnsScreen(
    ...
    ScreenList(padding) {
        ...
        item {
            CardBlock {
                Text("DNS lookup checker", fontWeight = FontWeight.SemiBold)
                OutlinedTextField(domain, { domain = it }, label = { Text("адрес/домен") }, modifier = Modifier.fillMaxWidth())
                OutlinedTextField(server, { server = it }, label = { Text("DNS server") }, modifier = Modifier.fillMaxWidth())
                OutlinedTextField(record, { record = it }, label = { Text("A / AAAA") })
                Text("Профиль: ${config.profiles.firstOrNull { it.id == config.defaultProfileId }?.name ?: "не выбран"}")
                Button(onClick = { scope.launch { result = DnsDiagnostic().lookup(domain, server, record) } }) { Text("Проверить DNS") }
                Text(
                    text = "В этой версии Android может использовать системный резолвер. Прямой запрос к выбранному DNS будет улучшен позже.",
                    style = MaterialTheme.typography.bodySmall,
                )
                result?.let { DiagnosticCard("Результат DNS", it) }
            }
        }
        item {
            CardBlock {
                Text("Проверить приложение", fontWeight = FontWeight.SemiBold)
                Text("Выберите приложение и посмотрите, через какой DNS и маршрут должны идти его запросы.")
                OutlinedTextField(app, { app = it }, label = { Text("Приложение") }, modifier = Modifier.fillMaxWidth())
                val decision = RouteEngine(config).simulate(app)
                Text("Маршрут: ${decision.tunnelProfile.name}")
                Text("DNS policy: ${decision.dnsPolicySummary}")
                Text("Возможный риск: ${decision.dnsLeakSummary}", style = MaterialTheme.typography.bodySmall)
                Text("Реальные домены приложения будут видны в FS после включения локального VPN-режима.", style = MaterialTheme.typography.bodySmall)
            }
        }
        item {
            CardBlock {
                Text("hosts-like overrides", fontWeight = FontWeight.SemiBold)
                Text(
                    text = "Локальная привязка имени к IP, аналог hosts. В этой версии используется для конфигурации и симуляции; реальное применение в DNS engine будет добавлено позже.",
                    style = MaterialTheme.typography.bodySmall,
                )
                OutlinedTextField(host, { host = it }, label = { Text("hostname") }, modifier = Modifier.fillMaxWidth())
                OutlinedTextField(ip, { ip = it }, label = { Text("IP") }, modifier = Modifier.fillMaxWidth())
                Button(
                    onClick = {
                        if (host.isNotBlank() && ip.isNotBlank()) {
                            val override = DnsHostOverride("host_${System.currentTimeMillis()}", host, ip, true, null)
                            onConfig(config.copy(hostOverrides = config.hostOverrides + override), "Override добавлен.")
                            host = ""
                            ip = ""
                        }
                    },
                ) { Text("Добавить override") }
                config.hostOverrides.forEach { override ->
                    Row(
                        ...
                        Text(
                            text = "${if (override.enabled) "✓" else "—"} ${override.hostname} → ${override.ipAddress}",
                            modifier = Modifier.weight(1f),
                        )
                        Switch(
                            ...
                            onCheckedChange = {
                                onConfig(
                                    config.copy(hostOverrides = config.hostOverrides.map { if (it.id == override.id) it.copy(enabled = !it.enabled) else it }),
                                    null,
                                )
                            },
                        )
                        OutlinedButton(
                            onClick = { onConfig(config.copy(hostOverrides = config.hostOverrides.filterNot { it.id == override.id }), "Override удалён.") },
                        ) { Text("×") }
                    }
                }
            }
        ...
        item {
            CardBlock {
                Text("DNS per connection", fontWeight = FontWeight.SemiBold)
                config.profiles.forEach { profile ->
                    DnsPerProfileRow(profile, config, onConfig)
                }
            }
        }
    }
...
private fun DnsPerProfileRow(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(6.dp), modifier = Modifier.padding(vertical = 6.dp)) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            config.dnsPolicies.take(4).forEach { dnsPolicy ->
                FilterChip(
                    selected = dnsPolicy.id == profile.dnsPolicyId,
                    onClick = {
                        onConfig(
                            config.copy(profiles = config.profiles.map { if (it.id == profile.id) it.copy(dnsPolicyId = dnsPolicy.id) else it }),
                            "DNS профиль обновлён.",
                        )
                    },
                    label = { Text(dnsPolicy.type.label.take(10)) },
                )
            }
        }
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
            OutlinedTextField(custom, { custom = it }, label = { Text("Custom DNS") }, modifier = Modifier.weight(1f))
            Button(
                onClick = {
                    if (custom.isNotBlank()) {
                        val id = "dns_${System.currentTimeMillis()}"
                        val newPolicy = DnsPolicy(id, "Custom DNS ${profile.name}", DnsPolicyType.Custom, custom, profile.id, "Пользовательский DNS для симуляции.")
                        onConfig(
                            config.copy(
                                dnsPolicies = config.dnsPolicies + newPolicy,
                                profiles = config.profiles.map { if (it.id == profile.id) it.copy(dnsPolicyId = id) else it },
                            ),
                            "Custom DNS сохранён.",
                        )
                        custom = ""
...
private fun FlowScannerScreen(padding: PaddingValues) {
    ...
    ScreenList(padding) {
        ...
        item {
            CompactCard(
                title = "FS",
                simple = "FS показывает сетевые события после явного включения локального VPN-режима ViRouteFS. Скрытого перехвата нет.",
                details = "Демонстрационный режим: реального packet capture и фонового мониторинга нет.",
            )
        }
        item {
            CardBlock {
                OutlinedTextField(selectedApp, { selectedApp = it }, label = { Text("Приложение") }, modifier = Modifier.fillMaxWidth())
                Button(onClick = { analyzing = true }) { Text("Старт анализа") }
                AssistChip(onClick = {}, label = { Text(if (analyzing) "Демонстрационный режим" else "Ожидание") })
            }
        }
...
@Composable
private fun FlowEventCard(event: FlowEvent) {
    var details by rememberSaveable(event.target) { mutableStateOf(false) }

    CardBlock {
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            Column(modifier = Modifier.weight(1f)) {
                Text("App: ${event.app}", fontWeight = FontWeight.SemiBold)
                Text("Target: ${event.target}")
            }
            AssistChip(onClick = {}, label = { Text("demo") })
        }
        Text("Route: ${event.route} • DNS: ${event.dns}")
        Text("Why: ${event.why}", style = MaterialTheme.typography.bodySmall)
        OutlinedButton(onClick = { details = !details }) { Text("Детали") }
        if (details) {
            Text(
                text = "DNS: ${event.dns}\nTCP/TLS/SNI: будет показано после локального VPN-режима\nRoute decision: ${event.why}\nРекомендация: проверьте правило в Routes.",
                style = MaterialTheme.typography.bodySmall,
            )
        }
    }
    ...
    ScreenList(padding) {
        ...
        item {
            CompactCard(
                ...
                simple = "Проверяйте только свои ресурсы или сети, где у вас есть разрешение.",
                details = "Инструменты запускаются вручную и не выполняют скрытого мониторинга.",
            )
        }
        item {
            CardBlock {
                Text("TCP / TLS", fontWeight = FontWeight.SemiBold)
                OutlinedTextField(host, { host = it }, label = { Text("Host") }, modifier = Modifier.fillMaxWidth())
                OutlinedTextField(port, { port = it }, label = { Text("Port") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number))
                OutlinedTextField(sni, { sni = it }, label = { Text("SNI") }, modifier = Modifier.fillMaxWidth())
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(onClick = { scope.launch { tcp = TcpDiagnostic().check(host, port, "5") } }) { Text("TCP check") }
                    Button(onClick = { scope.launch { tls = TlsDiagnostic().check(host, port, sni) } }) { Text("TLS/SNI check") }
                }
                tcp?.let { DiagnosticCard("TCP result", it) }
                tls?.let { DiagnosticCard("TLS result", it) }
            }
        }
        item {
            CardBlock {
                Text("HTTP", fontWeight = FontWeight.SemiBold)
                OutlinedTextField(url, { url = it }, label = { Text("URL") }, modifier = Modifier.fillMaxWidth())
                Button(onClick = { scope.launch { http = HttpDiagnostic().check(url) } }) { Text("HTTP check") }
                http?.let { DiagnosticCard("HTTP result", it) }
            }
        }
        item {
            CardBlock {
                val decision = RouteEngine(config).simulate(routeTarget)
                Text("Route diagnostics", fontWeight = FontWeight.SemiBold)
                OutlinedTextField(routeTarget, { routeTarget = it }, label = { Text("домен/IP/приложение") }, modifier = Modifier.fillMaxWidth())
                Text("${decision.input} → ${decision.tunnelProfile.name} • ${decision.dnsPolicySummary}")
            }
        }
    ...
    ScreenList(padding) {
        ...
        item {
            CardBlock {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    listOf("Русский", "English", "中文简体").forEach { option ->
                        FilterChip(selected = language == option, onClick = { language = option }, label = { Text(option) })
                    }
                }
                Text("Полное переключение языка будет добавлено позже.")
                Text(
                    text = "Roadmap: فارسی / Persian • Türkçe / Turkish • العربية / Arabic • Español / Spanish • Português / Portuguese • Bahasa Indonesia • Deutsch",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
        }
        item {
            CardBlock {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    listOf("System", "Light", "Dark", "AMOLED black").forEach { option ->
                        FilterChip(selected = theme == option, onClick = { theme = option }, label = { Text(option) })
                    }
                }
                Text("AMOLED black будет означать настоящий чёрный фон в будущей теме.", style = MaterialTheme.typography.bodySmall)
            }
        }
        item {
            CardBlock {
                Text("Поддержать проект", fontWeight = FontWeight.SemiBold)
                Text("ViRouteFS — свободное open-source приложение без рекламы, телеметрии и облачной зависимости. Поддержка помогает развивать Android, Linux и Windows версии.")
...
private fun DiagnosticCard(
    ...
    CompactCard(
        title = title,
        simple = result.simpleExplanation,
        details = result.technicalDetailsWithElapsed() + "\nЧто сделать: " + result.recommendedAction,
    )
}
