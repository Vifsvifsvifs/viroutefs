    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        InfoCard(
            InfoCardContent(
                ...
                simpleExplanation = "${routeDecision.input} → ${routeDecision.tunnelProfile.name}",
                technicalDetails = routeDecision.technicalDetails,
        ...
        Card(
            ...
            Column(
                ...
                LabeledText(label = "Профиль", body = "${routeDecision.tunnelProfile.name} (${routeDecision.tunnelProfile.type.label})")
                LabeledText(label = "Правило", body = "${routeDecision.matchedRule.name}, приоритет ${routeDecision.matchedRule.priority}")
                LabeledText(label = "DNS-политика", body = routeDecision.dnsPolicySummary)
                LabeledText(label = "Почему выбран", body = routeDecision.plainReason)
                LabeledText(label = "DNS и утечки", body = routeDecision.dnsLeakSummary)
                if (routeDecision.tunnelProfile.mockOnly) {
                    LabeledText(label = "Ограничение профиля", body = routeDecision.profileMockSummary)
                }
                routeDecision.warnings.forEach { warning ->
                    LabeledText(label = "Предупреждение", body = warning)
                }
            }
        }
    }
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Card(
            ...
            Column(
                ...
                LabeledText(label = "DNS-политика", body = report.dnsPolicy)
        ...
        InfoCard(
            InfoCardContent(
                ...
                technicalDetails = "Реальное VPN-маршрутизирование и DNS-маршрутизирование не подключены в этом отчёте.",
                recommendedAction = report.recommendedNextAction,
            ),
        )
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
...
private fun Context.copyRouteReport(report: RouteDiagnosticReport) {
    copyText("ViRouteFS route report", report.toPlainText())
}
